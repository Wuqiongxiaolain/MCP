...
inline bool isValidId(const std::string& id) {
    ...
    for (size_t i = 0; i < id.size(); ++i) {
        ...
        if (c == '.' && i + 1 < id.size() && id[i + 1] == '.') return false;
    }
    return true;
}
...
inline OpType opTypeFromString(const std::string& s) {
    if (s == "NODE_INSERT") return OpType::NODE_INSERT;
    if (s == "NODE_UPDATE") return OpType::NODE_UPDATE;
    if (s == "NODE_DELETE") return OpType::NODE_DELETE;
    if (s == "EDGE_INSERT") return OpType::EDGE_INSERT;
    if (s == "EDGE_UPDATE") return OpType::EDGE_UPDATE;
    if (s == "EDGE_DELETE") return OpType::EDGE_DELETE;
    if (s == "META_UPDATE") return OpType::META_UPDATE;
    return OpType::UNKNOWN;
}

// ─── FieldChange: 单个字段的变更记录 ──────────────────────────────
struct FieldChange {
    std::string field;     // 字段名：label / shape / style / x / y / w / h / parent / from / to / arrow
...
struct Operation {
    ...
    std::string summary() const {
        std::string s = std::string(opTypeName(type)) + " " + targetType + " " + targetId;
        if (type == OpType::NODE_UPDATE || type == OpType::EDGE_UPDATE) {
            if (!changes.empty()) s += " (" + std::to_string(changes.size()) + " fields)";
        }
        return s;
    }
    ...
    Json toJson() const {
        ...
        if (!changes.empty()) {
            Json arr = Json::arr();
            for (auto& c : changes) arr.push(c.toJson());
            j.set("changes", arr);
        }
        if (snapshot.isObj() || snapshot.isArr()) j.set("snapshot", snapshot);
    ...
    static Operation fromJson(const Json& j) {
        ...
        if (const Json* ch = j.find("changes")) {
            if (ch->isArr()) for (auto& c : *ch->a)
                op.changes.push_back(FieldChange::fromJson(c));
        }
        if (const Json* sn = j.find("snapshot")) op.snapshot = *sn;
...
struct Draft {
    ...
    int    operationCount() const { return (int)operations.size(); }
    ...
    Json toJson() const {
        ...
        Json ops = Json::arr();
        for (auto& op : operations) ops.push(op.toJson());
...
struct Stage {
    ...
    bool isEmpty() const { return stagedOpIndices.empty(); }

    Json toJson() const {
        Json j = Json::obj();
        j.set("graphId", graphId);
        Json idx = Json::arr();
        for (int i : stagedOpIndices) idx.push(Json((double)i));
...
struct Commit {
    ...
    Json toJson() const {
        ...
        j.set("timestamp", timestamp);
        Json pa = Json::arr();
        for (auto& op : patch) pa.push(op.toJson());
        j.set("patch", pa);
    ...
    static Commit fromJson(const Json& j) {
        ...
        if (const Json* pa = j.find("patch")) {
            if (pa->isArr()) for (auto& op : *pa->a)
                c.patch.push_back(Operation::fromJson(op));
        }
        if (const Json* m = j.find("model")) c.modelSnapshot = *m;
        return c;
    }

    // 从 parent + patch 重建完整模型
    static Graph rebuild(const Graph& parentModel, const std::vector<Operation>& patch);
};
...
struct Selector {
    ...
    static Selector byId(const std::string& id) {
        Selector s; s.kind = Kind::BY_ID; s.value = id; return s;
    }
    static Selector byType(const std::string& shape) {
        Selector s; s.kind = Kind::BY_TYPE; s.value = shape; return s;
    }
    static Selector byLabel(const std::string& label, bool fuzzy = false) {
        Selector s; s.kind = Kind::BY_LABEL; s.value = label; s.regex = fuzzy; return s;
    }
    static Selector byParent(const std::string& parentId) {
        Selector s; s.kind = Kind::BY_PARENT; s.value = parentId; return s;
    }
    static Selector allNodes() {
        Selector s; s.kind = Kind::ALL_NODES; return s;
    }
    static Selector allEdges() {
        Selector s; s.kind = Kind::ALL_EDGES; return s;
    }
    static Selector connectedTo(const std::string& nodeId) {
        Selector s; s.kind = Kind::CONNECTED_TO; s.value = nodeId; return s;
    }
...
inline std::string getNodeField(const Node& n, const std::string& field) {
    if (field == "id")     return n.id;
    if (field == "label")  return n.label;
    if (field == "shape")  return n.shape;
    if (field == "parent") return n.parent;
    if (field == "style")  return n.style;
    if (field == "x")      return std::to_string(n.x);
    if (field == "y")      return std::to_string(n.y);
    if (field == "w")      return std::to_string(n.w);
    if (field == "h")      return std::to_string(n.h);
    return "";
}

inline void setNodeField(Node& n, const std::string& field, const std::string& val) {
    if      (field == "label")  n.label  = val;
    else if (field == "shape")  n.shape  = val;
    else if (field == "parent") n.parent = val;
    else if (field == "style")  n.style  = val;
    else if (field == "x")      n.x = std::strtod(val.c_str(), nullptr);
    else if (field == "y")      n.y = std::strtod(val.c_str(), nullptr);
    else if (field == "w")      n.w = std::strtod(val.c_str(), nullptr);
    else if (field == "h")      n.h = std::strtod(val.c_str(), nullptr);
}

inline std::string getEdgeField(const Edge& e, const std::string& field) {
    if (field == "id")    return e.id;
    if (field == "from")  return e.from;
    if (field == "to")    return e.to;
    if (field == "label") return e.label;
    if (field == "style") return e.style;
    if (field == "arrow") return e.arrow;
    return "";
}

inline void setEdgeField(Edge& e, const std::string& field, const std::string& val) {
    if      (field == "from")  e.from  = val;
    else if (field == "to")    e.to    = val;
    else if (field == "label") e.label = val;
    else if (field == "style") e.style = val;
    else if (field == "arrow") e.arrow = val;
}
...
inline bool selectorMatchesNode(const Node& n, const Selector& sel) {
    switch (sel.kind) {
        case Selector::Kind::BY_ID:
            return n.id == sel.value;
        case Selector::Kind::BY_TYPE:
            return n.shape == sel.value;
        case Selector::Kind::BY_LABEL:
            return sel.regex ? (n.label.find(sel.value) != std::string::npos)
                             : (n.label == sel.value);
        case Selector::Kind::BY_PARENT:
            return n.parent == sel.value;
        case Selector::Kind::ALL_NODES:
            return true;
...
// ─── Commit::rebuild ─────────────────────────────────────────────
inline Graph Commit::rebuild(const Graph& parentModel, const std::vector<Operation>& patch) {
    Graph g = parentModel;
    for (auto& op : patch) {
        if (op.type == OpType::NODE_INSERT) {
            ...
            g.edges.erase(
                std::remove_if(g.edges.begin(), g.edges.end(),
                    [&](const Edge& e) { return e.id == op.targetId; }),
                g.edges.end());
        } else if (op.type == OpType::META_UPDATE) {
            for (auto& ch : op.changes) {
                if (ch.field == "name") g.name = ch.newValue;
                else if (ch.field == "type") g.type = ch.newValue;
            }
        }
    }
    return g;
...
inline Json nodeToSnapshot(const Node& n) {
    ...
    if (!n.attrs.empty()) {
        Json arr = Json::arr();
        for (auto& a : n.attrs) arr.push(Json(a));
        j.set("attrs", arr);
    }
    j.set("x", n.x);
    j.set("y", n.y);
...
inline Json edgeToSnapshot(const Edge& e) {
    ...
    j.set("from", e.from);
    j.set("to", e.to);
    if (!e.label.empty()) j.set("label", e.label);
    j.set("style", e.style);
    j.set("arrow", e.arrow);
    return j;
}
