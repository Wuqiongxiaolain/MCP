...
public:
    ...
    size_t size() const {
        if (t == ARR && a) return a->size();
        if (t == OBJ && o) return o->size();
        return 0;
    }
    ...
    void dumpTo(std::string& out, int indent, int depth) const {
        ...
        switch (t) {
            ...
            case STR: out += '"'; escape(s, out); out += '"'; break;
            case ARR: {
                out += '[';
                bool first = true;
                if (a) for (auto& v : *a) {
    ...
    struct Parser {
        ...
        bool parseValue(Json& out) {
            ...
            if (c == '"') { out.t = STR; return parseString(out.s); }
            if (c == 't') { if (src.compare(pos, 4, "true") == 0)  { pos += 4; out = Json(true);  return true; } return fail("bad literal"); }
            if (c == 'f') { if (src.compare(pos, 5, "false") == 0) { pos += 5; out = Json(false); return true; } return fail("bad literal"); }
        ...
        bool parseString(std::string& out) {
            ...
            while (pos < src.size()) {
                ...
                if (c == '\\') {
                    pos++;
                    if (pos >= src.size()) return fail("bad escape");
                    char e = src[pos++];
                    switch (e) {
                        case '"':  out += '"';  break;
                        case '\\': out += '\\'; break;
        ...
        bool parseObject(Json& out) {
            ...
            while (true) {
                skipWs();
                std::string key;
                if (pos >= src.size() || src[pos] != '"') return fail("expected object key");
