...
// ===================================================================
// Args: 命令行参数解析结果
// family    = 命令族 (create/convert/export/edit/layout/validate/store/version/graph/serve)
...
Args parseArgs(int argc, char** argv) {
    Args a;
    if (argc >= 2) a.family = argv[1];
    if (argc >= 3 && argv[2][0] != '-') a.subcommand = argv[2];
...
std::string getStoreDir(const Args& a) {
    if (a.has("store")) return a.get("store");
...
std::string getGraphId(const Args& a) {
    if (a.has("id")) return a.get("id");
    if (!a.positionals.empty()) return a.positionals[0];
    return "";
}
...
int usage(const std::string& hint = "", int exitCode = 1) {
    ...
    std::cout <<
        ...
        "\n"
        "command families:\n"
        "  create    from-{mermaid|markdown|csv|xml|excalidraw|model|input}\n"
        ...
        "\n"
        "formats:\n"
        "  input : mermaid | markdown | csv | xml | excalidraw | model | auto\n"
        "  output: drawio | mermaid | excalidraw | svg | png | pdf | url | model\n"
        "\n"
        "examples:\n"
        "  graphmcp create from-mermaid --file flow.mmd --name \"login\"\n"
        "  graphmcp convert to-svg --content \"A-->B\" --input-format mermaid --stdout\n"
        "  graphmcp store list\n"
        "  graphmcp version commit g7abc -m \"add error handling\"\n"
        "  graphmcp graph update --node A --set label=\"Start\"\n"
        "  graphmcp graph insert --node --type rect --label \"Step\" --position 400 200\n"
        "  graphmcp serve\n"
        "\n"
...
bool handleLegacyCommand(const std::string& command, Args& a, gs::Store& store) {
    ...
    if (command == "create" && a.subcommand.empty()) {
        Graph g = gp::parseAny(readInput(a), a.get("format", "auto"), a.get("type"));
        if (a.has("name")) g.name = a.get("name");
        if (a.has("id"))   g.id   = a.get("id");
        auto issues = gl::validate(g);
        if (!issues.empty()) printIssues(issues);
        ...
        std::cout << "created graph '" << g.name << "' id=" << g.id
                  << " v" << v << " (" << g.nodes.size() << " nodes, "
    ...
    if (command == "convert" && a.subcommand.empty()) {
        if (!a.has("to")) { usage("missing --to"); exit(1); }
        Graph g = gp::parseAny(readInput(a), a.get("format", "auto"), a.get("type"));
        ge::ExportResult r = ge::exportGraph(g, a.get("to"), a.get("output"));
        if (!r.ok) { std::cerr << "error: " << r.message << "\n"; exit(4); }
        if (!r.content.empty()) std::cout << r.content << "\n";
        else std::cout << r.message << "\n";
        return true;
    }

    // export (旧版)
    if (command == "export" && a.subcommand.empty()) {
        if (!a.has("id") || !a.has("to")) { usage("missing --id or --to"); exit(1); }
        Graph g;
        std::string err;
        if (!store.load(a.get("id"), g, atoi(a.get("version", "0").c_str()), &err)) {
            std::cerr << "error: " << err << "\n"; exit(5);
        }
        ge::ExportResult r = ge::exportGraph(g, a.get("to"), a.get("output"));
        if (!r.ok) { std::cerr << "error: " << r.message << "\n"; exit(4); }
    ...
    if (command == "open" && a.subcommand.empty()) {
        if (!a.has("id")) { usage("missing --id"); exit(1); }
    ...
    if (command == "validate" && a.subcommand.empty()) {
        ...
        auto issues = gl::validate(g);
        if (issues.empty()) { std::cout << "valid: no issues found\n"; return true; }
        printIssues(issues);
    ...
    if (command == "list" && a.subcommand.empty()) {
        ...
        if (idx["graphs"].size() == 0) std::cout << "(store is empty)\n";
        return true;
    }
    ...
    if (command == "history" && a.subcommand.empty()) {
        ...
        for (auto& e : *h.a) {
            std::cout << "v" << (int)e.num("version") << "  " << e.str("savedAt")
                      << "  nodes=" << (int)e.num("nodes")
                      << " edges=" << (int)e.num("edges");
            if (!e.str("note").empty()) std::cout << "  (" << e.str("note") << ")";
            std::cout << "\n";
        }
        return true;
    }
    ...
    if (command == "rollback" && a.subcommand.empty()) {
        if (!a.has("id") || !a.has("version")) { usage("missing --id or --version"); exit(1); }
        int nv = 0; std::string err;
        if (!store.rollback(a.get("id"), atoi(a.get("version").c_str()), &nv, &err)) {
            std::cerr << "error: " << err << "\n"; exit(5);
        }
        std::cout << "rolled back " << a.get("id") << " to v" << a.get("version")
                  << " (saved as v" << nv << ")\n";
        return true;
    }
...
int cmdCreate(Args& a, gs::Store& store) {
    ...
    if (fmt.empty()) { usage("create requires subcommand"); return 1; }

    // from-mermaid / from-markdown / from-csv / from-xml / from-excalidraw / from-model / from-input
    std::string inputFormat;
    if      (fmt == "from-mermaid")    inputFormat = "mermaid";
    else if (fmt == "from-markdown")   inputFormat = "markdown";
    else if (fmt == "from-csv")        inputFormat = "csv";
    else if (fmt == "from-xml")        inputFormat = "xml";
    else if (fmt == "from-excalidraw") inputFormat = "excalidraw";
    else if (fmt == "from-model")      inputFormat = "model";
    else if (fmt == "from-input")      inputFormat = "auto";
    else { usage("unknown create subcommand: " + fmt); return 1; }

    Graph g = gp::parseAny(readInput(a), a.get("input-format", inputFormat), a.get("type"));
    if (a.has("name")) g.name = a.get("name");
    if (a.has("id"))   g.id   = a.get("id");
    ...
    if (doValidate) {
        ...
        if (!issues.empty()) printIssues(issues);
        if (gl::hasErrors(issues)) {
            std::cerr << "rejected: graph has structural errors (use --no-validate to force)\n";
            return 3;
        }
    }
...
int cmdConvert(Args& a) {
    ...
    if (targetFmt.empty()) { usage("convert requires to-<format> subcommand"); return 1; }

    Graph g = gp::parseAny(readInput(a), a.get("input-format", "auto"));
    if (!a.has("no-layout") && targetFmt != "url") gl::layout(g);

    std::string outPath = a.get("output");
    if (a.has("stdout")) outPath = "";
    ge::ExportResult r = ge::exportGraph(g, targetFmt, outPath);
    if (!r.ok) { std::cerr << "error: " << r.message << "\n"; return 4; }
    if (!r.content.empty()) std::cout << r.content << "\n";
    else if (!r.message.empty()) std::cout << r.message << "\n";
    return 0;
}
...
int cmdExport(Args& a, gs::Store& store) {
    ...
    if (a.subcommand.size() > 3 && a.subcommand.compare(0, 3, "to-") == 0)
        targetFmt = a.subcommand.substr(3);
    if (targetFmt.empty()) { usage("export requires to-<format> subcommand"); return 1; }
...
int cmdEdit(Args& a, gs::Store& store) {
    ...
    if (editor.empty()) { usage("edit requires with-<editor> subcommand"); return 1; }
...
int cmdLayout(Args& a, gs::Store& store) {
    ...
    if (strat == "auto" || strat == "layered" || strat == "tree-h" ||
        ...
        usage("unknown layout subcommand: " + a.subcommand); return 1;
    }
    ...
    if (a.has("save")) {
        ...
        std::cout << "layout applied (" << g.nodes.size() << " nodes positioned)\n";
    }
    return 0;
}
...
int cmdValidate(Args& a, gs::Store& store) {
    ...
    for (auto& i : issues) {
        if (quiet && i.severity == "warning") continue;
        std::cout << "  [" << i.severity << "] " << i.message << "\n";
    }
    if (strict) return gl::hasErrors(issues) ? 3 : (issues.empty() ? 0 : 1);
...
int cmdStore(Args& a, gs::Store& store) {
    if (a.subcommand == "list") {
        ...
        if (count == 0) std::cout << "(store is empty)\n";
        else std::cout << count << " graph(s)\n";
        return 0;
    }
...
int cmdVersion(Args& a, gs::Store& store) {
    ...
    if (a.subcommand == "status") {
        ...
        std::cout << "  Status:   " << (s.dirty ? "⚠ uncommitted changes" : "✓ working tree clean") << "\n";
        return 0;
    }

    if (a.subcommand == "draft") {
        if (id.empty()) { usage("missing graph id"); return 1; }
        std::string action = a.positionals.size() > 1 ? a.positionals[1] : "show";
        if (action == "reset") {
    ...
    if (a.subcommand == "stage") {
        if (id.empty()) { usage("missing graph id"); return 1; }
        std::string action = a.positionals.size() > 1 ? a.positionals[1] : "add";
        if (action == "show") {
            auto stage = vm.loadStage(id);
            if (stage.isEmpty()) { std::cout << "Stage is empty\n"; return 0; }
            std::cout << "Staged " << stage.stagedOpIndices.size() << " operations:\n";
            for (int idx : stage.stagedOpIndices)
                std::cout << "  [" << idx << "]\n";
            if (!stage.message.empty()) std::cout << "message: " << stage.message << "\n";
            return 0;
        }
        if (action == "clear") {
            vm.clearStage(id);
        ...
        if (a.has("select")) {
            ...
            std::cout << "staged " << indices.size() << " operation(s) for " << id << "\n";
        } else {
            vm.stageAll(id);
            auto draft = vm.loadDraft(id);
            std::cout << "staged " << draft.operationCount() << " operation(s) for " << id << "\n";
        }
        return 0;
    }
    ...
    if (a.subcommand == "commit") {
        ...
        if (msg.empty()) { usage("commit requires -m <message>"); return 1; }
        int nv;
        if (a.has("all")) {
    ...
    if (a.subcommand == "log") {
        ...
        if (hist.empty()) { std::cerr << "no history for graph: " << id << "\n"; return 1; }
    ...
    if (a.subcommand == "show") {
        ...
        if (commit.version == 0) {
            ...
            if (g.id.empty()) { std::cerr << "graph not found: " << id << "\n"; return 5; }
            std::cout << "Graph: " << g.name << " (" << g.id << ")\n";
            std::cout << "  type: " << g.type << "\n";
            std::cout << "  nodes: " << g.nodes.size() << "\n";
            std::cout << "  edges: " << g.edges.size() << "\n";
    ...
    if (a.subcommand == "diff") {
        ...
        for (auto& op : ops) {
            std::string prefix;
            if (op.type == gv::OpType::NODE_INSERT || op.type == gv::OpType::EDGE_INSERT)
                { prefix = "  +"; nAdd++; }
            else if (op.type == gv::OpType::NODE_DELETE || op.type == gv::OpType::EDGE_DELETE)
                { prefix = "  -"; nDel++; }
            else
                { prefix = "  ~"; nMod++; }
    ...
    if (a.subcommand == "checkout") {
        ...
        if (!vm.checkout(id, ver, force)) {
            std::cerr << "cannot checkout: draft has uncommitted changes.\n"
...
int cmdGraph(Args& a, gs::Store& store) {
    ...
    if (a.subcommand == "show") {
        if (a.has("node")) {
            ...
            const Node* n = nc.get();
            if (!n) { std::cerr << "node not found: " << a.get("node") << "\n"; return 5; }
            std::cout << "Node: " << n->id << "\n"
                      << "  label:  " << n->label << "\n"
                      << "  shape:  " << n->shape << "\n"
                      << "  parent: " << (n->parent.empty() ? "(root)" : n->parent) << "\n"
                      << "  style:  " << (n->style.empty() ? "(none)" : n->style) << "\n"
                      << "  pos:    (" << n->x << ", " << n->y << ")\n"
                      << "  size:   " << n->w << " × " << n->h << "\n";
            if (!n->attrs.empty()) {
                for (auto& attr : n->attrs) std::cout << "  attr:   " << attr << "\n";
            }
        } else if (a.has("edge")) {
            auto ec = gv::selectEdge(g, nullptr, a.get("edge"));
            const Edge* e = ec.get();
            if (!e) { std::cerr << "edge not found: " << a.get("edge") << "\n"; return 5; }
            std::cout << "Edge: " << e->id << "\n"
                      << "  from:  " << e->from << "\n"
                      << "  to:    " << e->to << "\n"
            ...
            std::cout << "Nodes:\n";
            for (auto& n : g.nodes) {
                std::cout << "  " << n.id << "  " << n.shape << "  \"" << n.label << "\"";
                if (!n.parent.empty()) std::cout << "  parent=" << n.parent;
                std::cout << "\n";
            }
            std::cout << "\nEdges:\n";
            for (auto& e : g.edges) {
    ...
    if (a.subcommand == "update") {
        ...
        if (setVals.empty()) { usage("update requires --set key=value"); return 1; }

        if (a.has("node")) {
            auto nc = gv::selectNode(g, draftPtr, a.get("node"));
            if (!nc.valid()) { std::cerr << "node not found: " << a.get("node") << "\n"; return 5; }
            for (auto& sv : setVals) {
                size_t eq = sv.find('=');
            ...
            if (!ec.valid()) { std::cerr << "edge not found: " << a.get("edge") << "\n"; return 5; }
            for (auto& sv : setVals) {
                size_t eq = sv.find('=');
                if (eq == std::string::npos) continue;
            ...
            if (sc.count() == 0) { std::cerr << "no elements matched\n"; return 1; }
            for (auto& sv : setVals) {
                size_t eq = sv.find('=');
                if (eq == std::string::npos) continue;
                sc.setAll(sv.substr(0, eq), sv.substr(eq + 1));
            }
            std::cout << "updated " << sc.count() << " element(s) matching: "
                      << a.get("selector") << "\n";
        } else {
        ...
        std::cout << "(draft saved: " << draft.operationCount() << " total operation(s))\n";
        return 0;
    }
    ...
    if (a.subcommand == "insert") {
        if (a.has("node")) {
            ...
            std::string nid = gv::insertNode(g, draftPtr, shape, label, x, y, w, h, parent);
            vm.saveDraft(id, draft);
            std::cout << "inserted node " << nid << ": " << shape << " \"" << label << "\"\n";
        } else if (a.has("edge")) {
            ...
            if (from.empty() || to.empty()) {
                usage("insert edge requires --from and --to"); return 1;
            }
            std::string eid = gv::insertEdge(g, draftPtr, from, to, label, style, arrow);
            vm.saveDraft(id, draft);
            std::cout << "inserted edge " << eid << ": " << from << " → " << to << "\n";
        } else if (a.has("attr")) {
            std::string nid = a.get("node");
            std::string val = a.get("value");
            if (nid.empty() || val.empty()) { usage("insert attr requires --node and --value"); return 1; }
            Node* n = g.findNode(nid);
            if (!n) { std::cerr << "node not found: " << nid << "\n"; return 5; }
    ...
    if (a.subcommand == "delete") {
        if (a.has("node")) {
            ...
            sc.deleteAll();
            vm.saveDraft(id, draft);
            std::cout << "deleted " << n << " element(s) matching: "
                      << a.get("selector") << "\n";
        } else {
...
int cmdCursor(Args& a, gs::Store& store) {
    ...
    if (cid.empty()) { usage("cursor requires --cursor <id> (from open)"); return 1; }
...
int main(int argc, char** argv) {
    ...
    if (a.family.empty() || a.has("help")) return usage("", 0);
    ...
    try {
        ...
        // ── 新版命令族分发 ──
        if (a.family == "create")    return cmdCreate(a, store);
        ...
        if (a.family == "version")   return cmdVersion(a, store);
        if (a.family == "graph")     return cmdGraph(a, store);
        if (a.family == "cursor")    return cmdCursor(a, store);
        if (a.family == "draft")     return cmdDraft(a, store);
