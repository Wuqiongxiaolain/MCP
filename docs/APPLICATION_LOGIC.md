...
通用表存储在 `graph-store/tables/`（`index.json` + `<tableId>/latest.json` + `versions/`），与图目录并列；交换格式为 **CSV** 与 **表 XML**（根 `<table>`），**不是**边表转图的 `parseCSV`，也**不是**图 XML（根 `<graph>`）。仓库内权威为 Table JSON。图↔表桥接为**有损投影**（见工具 `mode` / 列映射说明）。
...
- `table_create` / `table_import` 通过 `format`（默认 `csv`）选择解析器；不做 `auto` 探测。
...
已知债：与 CSV 装表样板可能少量重复；表模块依赖图解析头中的 `detail`。

出现下列**任一**情况时，应单独开重构 PR，转向「功能抽离与代码复用」，勿在双份装表或 `detail` 依赖上继续叠功能：
...
届时目标形态：抽出 `xml_util.hpp`（`XmlNode`/`parseXmlDoc`）供图/表共用；抽出 `buildTable` 供 `fromCsv`/`fromXml` 共用装表；映射层仍分离。回归须覆盖既有 CSV 用例与表 XML 往返用例。
---
...
├── tables/                       ← 通用表（CSV/表 XML 交换，JSON 存档；与图并列）
│   ├── index.json
│   └── <tableId>/
│       ├── HEAD
│       ├── latest.json
