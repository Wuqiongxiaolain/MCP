...
public:
    ...
    gs::Store& store() { return store_; }
    const std::string& root() const { return store_.root(); }
    ...
    Draft loadDraft(const std::string& graphId) {
        ...
        if (!txt.empty()) {
            ...
            if (err.empty()) return Draft::fromJson(j);
        }
        // 新建空草稿
        Draft d;
        d.graphId = graphId;
    ...
    Graph materializeDraft(const std::string& graphId) {
        ...
        if (draft.isEmpty()) {
            ...
            std::string err;
            Graph g;
            store_.load(graphId, g, draft.baseVersion > 0 ? draft.baseVersion : 0, &err);
            return g;
        }
        Graph base;
        std::string err;
        if (!store_.load(graphId, base, draft.baseVersion > 0 ? draft.baseVersion : 0, &err)) {
            // base 不存在，从空图开始
            base = Graph();
            base.id = graphId;
        }
    ...
    Stage loadStage(const std::string& graphId) {
        ...
        if (!txt.empty()) {
            std::string err;
            Json j = Json::parse(txt, &err);
            if (err.empty()) return Stage::fromJson(j);
        }
        Stage s;
        s.graphId = graphId;
        return s;
    ...
    Stage stageSelected(const std::string& graphId, const std::vector<int>& indices) {
        Draft draft = loadDraft(graphId);
        Stage stage;
        stage.graphId = graphId;
        for (int idx : indices) {
    ...
    // commit: 将暂存区提交为新版本
    int commit(const std::string& graphId, const std::string& message,
               const std::string& author = "cli") {
        (void)author;  // 预留：提交者信息，暂未写入存储
        Draft draft = loadDraft(graphId);
        Stage stage = loadStage(graphId);

        if (stage.isEmpty()) return -1;  // 无事可提交

        // 获取 parent 版本
        int parentVersion = draft.baseVersion;
        Graph base;
        std::string err;
        if (!store_.load(graphId, base, parentVersion > 0 ? parentVersion : 0, &err))
        ...
        std::string cid = shortHash(graphId + ":" + std::to_string(parentVersion) +
                                   ":" + message + ":" + nowIso());
        int newVersion = store_.save(committedModel, message, parentVersion, cid);

        // 先保存 draft（裁剪已提交操作 + 更新 baseVersion），再写 HEAD
        // 崩溃安全：draft 更新成功后 HEAD 才指向新版本
        std::set<int> staged(stage.stagedOpIndices.begin(), stage.stagedOpIndices.end());
        std::vector<Operation> remaining;
        for (int i = 0; i < draft.operationCount(); i++) {
            if (!staged.count(i)) remaining.push_back(draft.operations[i]);
        }
    ...
    int commitAll(const std::string& graphId, const std::string& message,
                  const std::string& author = "cli") {
        Draft draft = loadDraft(graphId);
        if (draft.isEmpty()) return -1;
    ...
    Commit show(const std::string& graphId, int version) {
        ...
        Graph g;
        std::string err;
        if (!store_.load(graphId, g, version, &err)) return c;
    ...
    std::vector<Operation> diff(const std::string& graphId, int v1, int v2) {
        Graph g1, g2;
        std::string err;
        if (!store_.load(graphId, g1, v1, &err)) return {};
        if (!store_.load(graphId, g2, v2, &err)) return {};

        std::vector<Operation> ops;

        // 构建节点 id 集合
        std::set<std::string> ids1, ids2;
        for (auto& n : g1.nodes) ids1.insert(n.id);
        for (auto& n : g2.nodes) ids2.insert(n.id);
        ...
        // 修改的节点
        for (auto& nid : ids1) {
            if (!ids2.count(nid)) continue;
            const Node* n1 = g1.findNode(nid);
            const Node* n2 = g2.findNode(nid);
            if (!n1 || !n2) continue;
            std::vector<FieldChange> changes;
            if (n1->label  != n2->label)  changes.push_back({"label",  n1->label,  n2->label});
            if (n1->shape  != n2->shape)  changes.push_back({"shape",  n1->shape,  n2->shape});
            if (n1->style  != n2->style)  changes.push_back({"style",  n1->style,  n2->style});
            if (n1->parent != n2->parent) changes.push_back({"parent", n1->parent, n2->parent});
            if (std::abs(n1->x - n2->x) > 0.01) changes.push_back({"x", std::to_string(n1->x), std::to_string(n2->x)});
            if (std::abs(n1->y - n2->y) > 0.01) changes.push_back({"y", std::to_string(n1->y), std::to_string(n2->y)});
            if (!changes.empty()) {
                Operation op;
        ...
        for (auto& e : g1.edges) eids1.insert(e.id);
        for (auto& e : g2.edges) eids2.insert(e.id);

        for (auto& eid : eids1)
            if (!eids2.count(eid)) {
                Operation op; op.type = OpType::EDGE_DELETE; op.targetId = eid;
                op.targetType = "edge"; ops.push_back(op);
            }
        for (auto& eid : eids2) {
            if (!eids1.count(eid)) {
                const Edge* e2 = nullptr;
                for (auto& e : g2.edges) if (e.id == eid) { e2 = &e; break; }
                Operation op; op.type = OpType::EDGE_INSERT; op.targetId = eid;
                op.targetType = "edge";
                if (e2) op.snapshot = edgeToSnapshot(*e2);
                ops.push_back(op);
            }
        }
        for (auto& eid : eids1) {
            if (!eids2.count(eid)) continue;
            const Edge* e1 = nullptr, *e2 = nullptr;
            for (auto& e : g1.edges) if (e.id == eid) { e1 = &e; break; }
            for (auto& e : g2.edges) if (e.id == eid) { e2 = &e; break; }
            if (!e1 || !e2) continue;
            std::vector<FieldChange> changes;
            if (e1->from  != e2->from)  changes.push_back({"from",  e1->from,  e2->from});
            if (e1->to    != e2->to)    changes.push_back({"to",    e1->to,    e2->to});
            if (e1->label != e2->label) changes.push_back({"label", e1->label, e2->label});
            if (e1->style != e2->style) changes.push_back({"style", e1->style, e2->style});
            if (e1->arrow != e2->arrow) changes.push_back({"arrow", e1->arrow, e2->arrow});
            if (!changes.empty()) {
                Operation op; op.type = OpType::EDGE_UPDATE; op.targetId = eid;
                op.targetType = "edge"; op.changes = changes; ops.push_back(op);
            }
        }
        return ops;
    }
    ...
    bool checkout(const std::string& graphId, int version, bool force = false) {
        Draft draft = loadDraft(graphId);
        if (!draft.isEmpty() && !force) return false; // 有未保存的修改
    ...
    Json draftStatus(const std::string& graphId) {
        ...
        auto countDiff = [](const auto& cur, const auto& old, auto eq) {
            ...
            for (auto& x : old) {
                bool present = false;
                for (auto& c : cur)
                    if (c.id == x.id) { present = true; break; }
                if (!present) removed++;
            }
        ...
        auto edgeEq = [](const gm::Edge& a, const gm::Edge& b) {
            return a.from == b.from && a.to == b.to &&
                   a.label == b.label && a.style == b.style &&
                   a.arrow == b.arrow;
        };
    ...
    int readHead(const std::string& id) {
        ...
        if (txt.empty()) {
            ...
            for (auto& item : *idx["graphs"].a) {
                if (item.str("id") == id) return (int)item.num("versions");
            }
            return 0;
        }
...
inline Json nodeToCursorJson(const gm::Node& n) {
    Json j = Json::obj();
    j.set("id", n.id); j.set("label", n.label); j.set("shape", n.shape);
    if (!n.parent.empty()) j.set("parent", n.parent);
    if (!n.style.empty())  j.set("style", n.style);
    return j;
}

inline Json edgeToCursorJson(const gm::Edge& e) {
    Json j = Json::obj();
    j.set("id", e.id); j.set("from", e.from); j.set("to", e.to);
    j.set("label", e.label); j.set("style", e.style); j.set("arrow", e.arrow);
    return j;
}
...
// ─── cursorOpen ──────────────────────────────────────────────────
inline Json openCursor(gs::Store& store, const std::string& graphId,
                       const std::string& targetIn) {
    if (!isValidId(graphId)) {
        Json e = Json::obj(); e.set("error", "invalid graph id");
        return e;
    }
    std::string target = targetIn.empty() ? "nodes" : targetIn;
    if (target != "nodes" && target != "edges") {
        Json e = Json::obj(); e.set("error", "target must be 'nodes' or 'edges'");
        return e;
    }
    Graph g; std::string err;
    if (!store.load(graphId, g, 0, &err)) {
        Json e = Json::obj(); e.set("error", "graph not found: " + graphId);
        return e;
    }
    ...
    Json st = Json::obj();
    st.set("graphId", graphId); st.set("target", target); st.set("index", 0);
    std::string dir = store.root() + "/" + graphId + "/cursors";
    gs::makeDir(dir);
    ge::writeFile(detail::cursorStatePath(store.root(), graphId, cid), st.dump(2));

    Json out = Json::obj();
    out.set("cursor", cid); out.set("graphId", graphId);
    out.set("target", target); out.set("index", 0);
...
// ─── cursorGet ───────────────────────────────────────────────────
inline Json getCursor(gs::Store& store, const std::string& graphId,
                      const std::string& cursorId) {
    if (!isValidId(graphId) || !isValidId(cursorId)) {
        Json e = Json::obj(); e.set("error", "invalid graph id or cursor id");
        return e;
    }
    std::string txt = ge::readFile(detail::cursorStatePath(store.root(), graphId, cursorId));
    if (txt.empty()) {
        Json e = Json::obj(); e.set("error", "cursor not found: " + cursorId); return e;
    }
    std::string perr; Json st = Json::parse(txt, &perr);
    if (!perr.empty()) {
        Json e = Json::obj(); e.set("error", "corrupt cursor state: " + perr); return e;
    }
    ...
    if (g.id.empty()) { std::string ign; store.load(graphId, g, 0, &ign); }

    int count = target == "nodes" ? (int)g.nodes.size() : (int)g.edges.size();
    Json out = Json::obj();
    out.set("target", target); out.set("index", (double)idx);
    out.set("count", (double)count);
    if (idx < 0 || idx >= count) {
        out.set("atEnd", true);
...
// ─── cursorMove ──────────────────────────────────────────────────
inline Json moveCursor(gs::Store& store, const std::string& graphId,
                       const std::string& cursorId, int delta) {
    if (!isValidId(graphId) || !isValidId(cursorId)) {
        Json e = Json::obj(); e.set("error", "invalid graph id or cursor id");
        return e;
    }
    ...
    if (newIdx > count) newIdx = count;

    st.set("index", (double)newIdx);
    ge::writeFile(detail::cursorStatePath(store.root(), graphId, cursorId), st.dump(2));

    Json out = Json::obj();
    out.set("target", target); out.set("index", (double)newIdx);
    out.set("count", (double)count);
...
// ─── cursorClose ─────────────────────────────────────────────────
inline Json closeCursor(gs::Store& store, const std::string& graphId,
                        const std::string& cursorId) {
    if (!isValidId(graphId) || !isValidId(cursorId)) {
        Json e = Json::obj(); e.set("error", "invalid graph id or cursor id");
        return e;
    }
    int rc = std::remove(detail::cursorStatePath(store.root(), graphId, cursorId).c_str());
    Json out = Json::obj();
    out.set("closed", rc == 0); out.set("cursor", cursorId);
    return out;
}
