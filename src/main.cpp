...
int usage() {
    std::cout <<
        ...
        "\n"
        "commands:\n"
        "  create    --input <file> [--format auto] [--type auto] [--name X]\n"
        "            parse + validate + layout + save to store; prints graph id\n"
        "  convert   --input <file> --to <fmt> [-o out]     one-shot conversion\n"
        ...
        "  history   --id <id>                             show version history\n"
        "  rollback  --id <id> --version N                 restore old version\n"
...
int main(int argc, char** argv) {
    ...
    for (auto& s : uargs) uptrs.push_back(const_cast<char*>(s.c_str()));
    if ((int)uptrs.size() == argc) argv = uptrs.data();
#endif
    ...
    try {
        ...
        if (a.command == "export") {
            if (!a.has("id") || !a.has("to")) return usage();
            Graph g;
            std::string err;
        ...
        if (a.command == "rollback") {
            if (!a.has("id") || !a.has("version")) return usage();
            int nv = 0;
            std::string err;
