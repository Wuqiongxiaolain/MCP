...
可用 MCP 直接改折点与微调几何：`graph_set_edge_route` / `graph_clear_edge_route` / `graph_nudge_node` / `graph_set_edge_heads`，或经 `graph_update` 写边字段 `waypoints`（无需整图 export/import model）。再次 `graph_layout` 且 `save=true` 仍会覆盖手改。
