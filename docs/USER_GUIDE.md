...
| 子命令 | 输入格式 | 示例文件 |
|--------|---------|---------|
| `from-mermaid` | Mermaid 文本（支持 19 种子类型：flowchart/mindmap/er/class/state/sequence/requirement/gantt/pie/sankey/kanban/gitGraph/journey/timeline/quadrantChart/xychart/block/packet/architecture） | `examples/flowchart.mmd` |
...
> **说明（v0.2.6）**：`layered` 会尽量减少边交叉并用 waypoint 画折线，但复杂图仍可能观感不佳；可用 `--force` 重排，或导出后在外部编辑器微调。也可用 MCP `graph_set_edge_route` / `graph_nudge_node` 等直接改折点与节点位置；整图 model 覆盖**可以**但是下下策。再次 `layout` 并保存仍会覆盖手改。
...
| 格式 | 对应子命令 | 说明 |
|------|-----------|------|
| Drawio XML | `to-drawio` | 可用 Draw.io 桌面版打开编辑；**支持多图层（layers）与多页（pages）往返** |
| Mermaid | `to-mermaid` | Mermaid 文本，可粘贴到 mermaid.live；flowchart 导出含 `classDef`/`linkStyle` 颜色指令 |
| Excalidraw | `to-excalidraw` | Excalidraw JSON，可导入 excalidraw.com；elements/files 保真往返 |
| SVG | `to-svg` | 矢量图，浏览器可直接打开；Excalidraw 精确路径 + 离线字体内嵌 |
| PNG | `to-png` | 位图（需要浏览器或 inkscape/rsvg/magick） |
| PDF | `to-pdf` | 文档格式（需要浏览器或转换工具） |
| URL | `to-url` | mermaid.live 在线编辑链接 |
| Model | `to-model` | 统一模型 JSON（含 layers/pages/properties），用于备份和二次处理 |
| Browser Page | `toMermaidBrowserPage()` | 内嵌 Mermaid.js 的完整 HTML 页面（当前仅 MCP/导出器内部路径） |
...
├─ 属性与游标 ───────────────────────────────────────┤
│ graphmcp graph show X --node A                     │
│ graphmcp graph_property get X --path pie           │
│ graphmcp cursor open --target nodes X              │
│ graphmcp cursor get --cursor <cid> X               │
│ graphmcp cursor next --cursor <cid> X              │
├─ 通用表 ───────────────────────────────────────────┤
│ graphmcp table create --file data.csv --name T     │
│ graphmcp table list                                │
│ graphmcp table show <id>                           │
│ graphmcp table update <id> --set-cells '[...]'     │
│ graphmcp table from-graph --graph-id X             │
│ graphmcp table check <id> --rules-id R             │
│ graphmcp table fix-enums <id> --rules-id R         │
├─ 服务与契约 ───────────────────────────────────────┤
│ graphmcp serve                                     │
│ graphmcp dump-tools --format openapi               │
└────────────────────────────────────────────────────┘
```
...
A: 使用 MCP `graph_update`（或 CLI `graph update`）改节点字段后 `graph_commit all=true` / `graph_apply`；图名等元数据可用版本草稿中的 `META_UPDATE` 路径（若客户端已封装）。导出 model JSON 手改再 `graph_import` **可以**，但是下下策，仅当原子工具无法表达该修改时再用。
...
| `GRAPHMCP_NO_LAUNCH` | 设为 `1` 时 `edit` 仅生成文件不拉起编辑器 |
| `GRAPHMCP_EDITOR` | 覆盖默认编辑器路径 |
| `GRAPHMCP_TABLE_CREATE_LEGACY_UPSERT` | 设为 `1` 时 `table create` 无需 `--force` 即可覆盖 |
| `GRAPHMCP_TABLE_CHECK_LEGACY_HINT` | 设为 `1` 时 `table_check` 默认不跳过 hint 行 |
...
graph-store/
  ...
  tables/                     # 通用表存储（并列一等）
    index.json                # 表目录索引
    <tableId>/
      HEAD / latest.json / versions/vN.json
  <图ID>/
    HEAD                      # 当前版本号
    latest.json               # 最新版本模型
    draft.json                # 草稿修改（操作序列）
    stage.json                # 暂存区
    cursors/                  # 持久化游标
    versions/
      v1.json                 # 不可变版本快照
